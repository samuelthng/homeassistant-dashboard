...
const lightReducer = (acc, state) => {
	...
	const [integration, deviceType, id, component = 'default'] = entity.split('_');
	return {
		...acc,
		[id]: {
			...acc[id],
			...
			integrations: new Set([...get(acc, [id, 'integrations'], []), integration]),
...
const YeeLightCards = () => {
	...
	return (
		<AutoFlowContainer>
			{lights.map(light => (
				<YeeLight key={light.id} state={light.state} entity_id={light.entity_id} id={light.id} />
			))}
		</AutoFlowContainer>
	);
};
